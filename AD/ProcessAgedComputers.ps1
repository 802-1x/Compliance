...
#Disables and moves aged Computer objects from the WK-Resources OU
$LastLogonDateCheck = $CurrentDate.AddDays($Days)
$AgedComputers = Get-ADComputer -SearchBase "OU"  -Filter {LastLogon -lt $LastLogonDateCheck -and Enabled -eq $true} -Properties Name,Description,lastlogon,distinguishedName,memberof | `
                 tee-object -file $ResultsLogPath$LogDate"_AgedComputers.log" | `
                 Move-ADObject -targetpath "OU" -PassThru | `
...
$DeleteComputers = Get-ADComputer -SearchBase "OU" -Filter {LastLogon -lt $WhenDisabledLogonDateCheck -and Enabled -eq $false} -Properties Name,Description,lastlogon,distinguishedName,memberof,useraccountcontrol | `
